% Code to plot simulation results from sm_testrig_lead_screw_friction
%% Plot Description:
%
...
if ~exist('simlog_sm_testrig_lead_screw_friction', 'var')
    sim('sm_testrig_lead_screw_friction')
end

% Reuse figure if it exists, else create new figure
if ~exist('h1_sm_testrig_lead_screw_friction', 'var') || ...
        ~isgraphics(h1_sm_testrig_lead_screw_friction, 'figure')
    h1_sm_testrig_lead_screw_friction = figure('Name', 'sm_testrig_lead_screw_friction');
end
figure(h1_sm_testrig_lead_screw_friction)
clf(h1_sm_testrig_lead_screw_friction)

temp_colororder = get(gca,'defaultAxesColorOrder');

% Get simulation results
simlog_t = simlog_sm_testrig_lead_screw_friction.Revolute_Screw.Rz.q.series.time;
simlog_qScrew = simlog_sm_testrig_lead_screw_friction.Revolute_Screw.Rz.q.series.values('rev');
simlog_zSlider = simlog_sm_testrig_lead_screw_friction.Prismatic_Slider.Pz.p.series.values('m');
simlog_trqAct = logsout_sm_testrig_lead_screw_friction.get('actuation_torque');
simlog_fLoad = logsout_sm_testrig_lead_screw_friction.get('load_force');
