function [xy_data_top, xy_data_side] = sm_extr_data_car_profile(whl_rad, whl_base, thk, varargin)
%sm_extr_data_car_profile Produce extrusion data for a car profile.
%   [xy_data_top, xy_data_side] = sm_extr_data_car_profile(whl_rad, whl_base, thk, varargin)
...
%   >> sm_extr_data_car_profile
%
%   To see a plot created with your parameter values,
%   add 'plot' as the final argument
%   >> sm_extr_data_car_profile(0.43, 3.2, 0.05,'plot')
...
xy_ell_hood = sm_extr_data_ellipse(1.2949,0.3548,180,360-45,1).*[-1 -1]+[-0.5854 0.2129+wrad_dl];
xy_ell_roof = sm_extr_data_ellipse(0.8514+wbase_dl/2,0.2483+wbase_dl/2*0.2483/0.8514,180+10,360-20,1).*[-1 -1]+[-2.1641-wbase_dl/2 0.7805+wrad_dl];
xy_trunk = [...
    -3.5477    0.7024;...
    -4.0798    0.6670]+[-wbase_dl wrad_dl];
xy_ell_rear1 = flipud(sm_extr_data_ellipse(0.1064,0.1419,120,205,1)).*[-1 -1]+[-4.1933-wbase_dl    0.6244+wrad_dl];
xy_ell_rear2 = flipud(sm_extr_data_ellipse(0.1206,0.2838,120,245,1)).*[-1 1].*[-1 -1]+[-4.0514-wbase_dl    0.2980+wrad_dl];
...
xy_ringwhlr = fliplr(sm_extr_data_ring(whl_rad,0,90,360-90)).*[-1 -1]+[-3.2000-wbase_dl 0];
xy_bottom1 = [3.9450-whl_rad 1.2772; 0.7450+whl_rad-wbase_dl 1.2772].*[-1 -1]+[0.7450-wbase_dl    1.0643];
xy_ringwhlf = fliplr(sm_extr_data_ring(whl_rad,0,90,360-90)).*[-1 -1];
...
car_top_outer_profile = sm_extr_data_cam_roller_curve(car_top_profile,thk,'outside');
...
if (nargin == 0 || strcmpi(showplot,'plot'))
    ...
    title('[xy\_data\_top, xy\_data\_side] = sm\_extr\_data\_car\_profile(whl\_rad, whl\_base, thk)'); 
